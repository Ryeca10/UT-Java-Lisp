        ...
        final Mapper map2 = new Mapper() 
        {
            public void map(String key, String value, MapReduce mr) 
            {
                ...
                int length = value.length();
            		...
            		{
            			if (Character.toLowerCase(((int)value.charAt(i))) > 96 && Character.toLowerCase((int)value.charAt(i)) < 123)
            				mr.collect_map( Character.toString(Character.toLowerCase((value.charAt(i)))),list("1"));
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	{
            		mr.collect_map(st.nextToken().toLowerCase(), list("1"));
            	}
            ...
            public void map(String key, String value, MapReduce mr) {
              ...
              // System.out.println("   grepkey = " + grepkey);
               ...
               for ( int i = 0 ; i < value.length() ; )
               {
            	   ...
            	   {
            		   for (int j = 0 ;  j < grepkey.length() ; j++)
                	   {
                		  
                		   if ( value.charAt(i) != grepkey.charAt(j)) 
                		   {
                			   b = false;
                			   
                			   if (value.charAt(i) == grepkey.charAt(0)) { break;}
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
               ...
               Cons newvalue = null;
                 for ( Cons lst = value ; lst != null ; lst = rest(lst))
                 {
                	 newvalue = cons(first((Cons)first(lst)),newvalue);
                 }
            	 mr.collect_reduce(key, nreverse(newvalue)); 
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	while (st.hasMoreTokens())
            	{
            		mr.collect_map(st.nextToken().toLowerCase(), list(key));
            	}
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	String rank = st.nextToken();
            		mr.collect_map(movie, list(rank));
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                ...
                {
                	count++;
               	 	sum += Double.parseDouble(first((Cons)first(lst)).toString());
                }
                
                Double average = sum / count;
                
               mr.collect_reduce(key, list(average.toString(),count));
