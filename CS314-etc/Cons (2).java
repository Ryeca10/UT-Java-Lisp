...
public static Cons solve(Cons e, String v) {
	...
	else if (consp(rhs((Cons)e)))
	{
		...
		else
		{
			...
			{
					e = list("=", list (inv, lhs((Cons)e), rhs((Cons)rhs(e))), lhs((Cons)rhs(e)));
					System.out.println("e of / is " + e.toString());
			}
    ...
    public static Cons vars (Object expr) {
    	...
    	else if (!numberp(expr) && !expr.equals("expt")
    							&& !expr.equals("exp")
    							&& !expr.equals("+")
    							&& !expr.equals("-")
    							&& !expr.equals("*")
    ...
    public static Double eval (Object tree, Cons bindings) {
    	...
    	if (consp(tree))
    	{
    		...
    		{		
    			if (rhs((Cons)tree) == null)
    			{
    				...
    				{
    					Double associatedNum = -1.0 * eval(lhs((Cons)tree),bindings);
    	...
    	else
    	{
    		if (numberp(tree))
    		{
    			Double d = (Double)(double)(int) tree;
    			return d;
    		}
    		else 
    		{
    			...
    			//System.out.println("bindings are " + bindings.toString());
    			return associatedNum;
    		}
    	}
...
public static Cons noValuesAssociated (Object expr, Cons bindings) {
    	...
    	for ( Cons arg = var ; arg != null; arg = rest (arg))
    	{
    		if (assoc(first(arg),bindings) == null)
    		{
    			result = cons (first(arg),result);
    		}
    	}
...
public static Double solveqns(Cons eqns, Cons vals, String v) {
	...
	/*for (Cons arg = eqns ; arg != null ; arg = rest(arg))
	{
		Cons vrs = vars(first(arg));
		Cons newEqns = solve();
	}*/
...
public static Double solveqnsb(Cons eqns, Cons eqnscpy, Cons vals, String v)
{
	Cons unknowns = null;
	Cons tupple;
	//System.out.println("eqns is " + eqns.toString());
	//System.out.println("eqnscpy is " + eqnscpy.toString());
	//System.out.println("vals is " + vals.toString());
	...
	if (consp(eqnscpy))
	{
		...
		{
			//System.out.println("rhs((Cons)first(eqnscpy)) is " 
								//+ rhs((Cons)first(eqnscpy)).toString());
			
			if (consp(rhs((Cons)first(eqnscpy))) &&
					(unknowns = noValuesAssociated(rhs((Cons)first(eqnscpy)),vals)) == null)
			{
				Double d = eval(rhs((Cons)first(eqnscpy)),vals);
				vals = cons(list(v,d),vals);
				
				return d;
			}
			else if ( stringp(lhs((Cons)first(eqnscpy))) && unknowns != null)
			{
				//System.out.println("we are here");
				Double d = solveqnsb(eqns, eqns, vals, (String)first(unknowns));
				//System.out.println("d is " + d);
				vals =cons(list((String)first(unknowns),d),vals);
				//System.out.println("vals are " + vals.toString());
				return solveqnsb(eqns, eqns, vals, v);
			}
				
			
			else if ( stringp(rhs((Cons)first(eqnscpy))) && 
						(tupple = assoc(rhs((Cons)first(eqnscpy)),vals)) != null)
			{
				Double d = (Double)lhs(tupple);
				return d;
			}
		...
		/*else if (consp(lhs((Cons)first(eqnscpy))))
		{
			//System.out.println("first(eqnscpy) is " + first(eqnscpy));
			Cons vrs = vars(first(eqnscpy));
			Cons newEqn;
			 
			if (member(v,vrs) == null)
			{
				return solveqnsb(eqns,rest(eqnscpy),vals,v);
			}
			else
			{	
				Cons noVal;
				newEqn = solve((Cons)first(eqnscpy),v);
				System.out.println("newEqn is " + newEqn.toString());
				eqns = cons(newEqn,eqns);
		...
		{
			if (rest(eqnscpy) != null)
			{
				eqnscpy = rest(eqnscpy);
				return solveqnsb(eqns, eqnscpy, vals, v);
			}
    ...
    public static Cons substitutions = readlist( list(
       ...
       "( (?function copytree) (?combine cons) (?baseanswer (if (tree != null) tree null)) )",
       "( (?function mintree) (?combine min)  (?baseanswer (if (tree != null) tree 999999)) )",
    ...
    public static Cons derivpats = readlist( list(
       ...
       "( (deriv (expt ?u ?v) ?x)  (* (deriv ?u ?x) (* ?v (expt ?u (- ?v 1))))",
       "( (deriv (log ?x) ?x)	(/ 1 (* ?x ln(2)))",
       "( (deriv (log ?u) ?x)   (* (deriv ?u ?x)  (/ 1 (* ?u ln(2)))))",

       "(   (deriv(* ?u ?v) ?x)  (+ (* ?u (deriv ?v ?x))(* ?v (deriv ?u ?x))))",
       "(	(deriv(sin ?x) ?x)  (cos ?x) )",
       "(	(deriv(sin (* ?u ?x)) ?x)  (* (deriv(* ?u ?x) ?x) (cos (* ?u ?x))) )",
       "(   (deriv(expt ?x 2) ?x)    (* 2 (expt ?x (- 2 1)))  )",
       "(   (deriv(expt ?x 3) ?x)    (* 3 (expt ?x (- 3 1)))  )",
       "(   (deriv(sqrt x?) ?x)     (/ 1 (sqrt ?x))                   ) ",  
       "(   (deriv(sqrt (expt ?x 2) ?x))  (/	(deriv(expt ?x ?k) ?x)  (* 2 (sqrt(expt ?x ?k)))  )",		
       "(   (deriv(sqrt (+ (expt ?x 2) ?b)) ?x)  (/ (deriv(expt ?x 2) ?x)  (* 2 (sqrt (+ (expt ?x 2) ?b)) )) ",
       "(	(deriv(log (expt ?u 3)) ?x)        (*   (deriv(expt ?u 3) ?x) (deriv (log ?u)))           )",
    ...
    public static Cons javapats = readlist( list(
       ...
       "( (pi) (Math.PI) )",
       "( (* ?x ?y) (zlparen ?x zspace * zspace ?y zrparen))",
       "( (+ ?x ?y)  (?x zspace + zspace ?y ; zreturn))",
       //"( (setq ?x (+ ?x 1)) (?x zspace += zspace 1 ; zreturn) )",
       //"( (if (> ?x 7) (setq y x) (if zspace zlparen zspace (?x zspace > zspace 7) zspace zrparen ztab zreturn (?y zspace = zspace ?x) ; zreturn)))",
       "( (setq x (+ x 1)) (x ++ ; zreturn))",

       "( (expt ?x ?y) (Math.pow zlparen ?x , zspace ?y zrparen) )",
       "( ((if (> ?x 7)) (setq y x)) (if zspace zlparen zlparen zspace ?x zspace < zspace ?y zrparen srparen ztab zreturn zlparen ?y zspace = zspace ?x zrparen ; zreturn))",
       // add more
