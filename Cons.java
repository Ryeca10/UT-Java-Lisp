...
public static String[] instruments = {"[Piano]", //0
                                      "[Piano]",  //1
                                      "[Piano]",  //2
                                      "[Piano]", //3
                                      "[bass_drum]", //4
...
// empty buffers into a music string
    ...
    public static Cons solve(Cons e, String v) 
    {
    	...
    	if (lhs(e).equals(v))
    		...
    		return e = list("=",rhs(e),lhs(e));
    	else if ( !consp(rhs(e)))
    		return null;
    	else
    	{
    		Cons result;
    		for (Cons arg = algpats ; arg != null ; arg = rest(arg))
    		{
    			Object trs = transform ((Cons)first(arg),e);
        		
        		if (trs != null)
        		{
        			
        			result = solve((Cons)trs,v);
        				if (result == null)
        					count++;
        				else
        					return result;
    ...
    public static int conshash(Object e) 
    {
    	...
    	if (consp(e))
    		...
    		return e.hashCode();
    	/*int hcf = 17 * (first((Cons)e)).hashCode();
    	int hcr = 0 ;
    	
    	for (Cons arg = rest((Cons)e) ; arg != null ; arg = rest(arg) )
    	{
    		hcr += 127 * (first((Cons)arg)).hashCode();
    	}
   ...
   public static int totaltime(Cons action)
   {
	   int max = 0;
		   ...
		   {
			   return (Integer)second(action) * totaltime((Cons)third(action));
		   }
		   else if  (first(action).equals("seq") ) 
	        {
			   Cons rest_of_seq = cons("seq",rest(rest(action)));
			  
			   if (rest(rest(action)) != null)
				   return totaltime((Cons)second(action)) + 
			    		totaltime(rest_of_seq);
			   else if (second(action) != null)
				   return totaltime((Cons)second(action));
			   else
				   return 0;
	        }
	        ...
	        {
			   if (rest(rest(action)) != null)
				   max = Math.max(totaltime(list("sync",rest(rest(action)))),
						          totaltime((Cons)second(action)));
			   else if (second(action) != null)
				   max = totaltime((Cons)second(action));
			   else
				   max = 0;
		   ...
		   else if (numberp(second(action)) && third(action) == null)
...
*/
    ...
    public static void execute(PriorityQueue pq, Cons act, int time) {
        ...
        {
        	int duration = (Integer)second(act)/4;
        	addevent(pq, list("boom",duration), time + duration);
        	addevent(pq, list("boom",duration * 2), time + duration * 2);
        	//addevent(pq, list("seq", list("rest",(Integer)second(act)/4)) , time + (Integer)second(act)/4);
        	//addevent(pq, list("boom", (Integer)second(act)/4) , time + (Integer)second(act)/4);
        	//addevent(pq, list("boom", (Integer)second(act)/2) , time + (Integer)second(act)/2);
        }
        ...
        else if  ( command.equals("seq") ) 
        {
        	...
        	//System.out.println("act is " + act.toString());
        	for (Cons arg = act ; arg != null ; arg = rest(arg))
        	{
        		if(consp(first(arg)))
        		{
	        		addevent(pq, (Cons)first(arg), time + tm);
	        		tm = totaltime((Cons)first(arg));
        		}
        	}
        }
        ...
        else if  ( command.equals("tambourine") )
        {
        	...
        	double tmp = random.nextDouble() * d;
        	int duration = (int) Math.floor(tmp);
        	...
        	{
        		d = duration;
        		tmp = random.nextDouble() * d;
        		duration = (int) Math.floor(tmp);
        ...
        {       		
 	        Cons action = (Cons) third (act);
 	        int n = (Integer) second (act);
 	        Cons newAct = list("repeat" , n-1, action );
 	       int tm = totaltime(action);
 
        	addevent(pq,action,time);
        	
        	if ( n > 1)
        	{
        		time = time + tm;
        		addevent(pq, newAct, time );
        	}
        }
        
        else if ( command.equals("sync") ) 
        {
        	//System.out.println("rest(act) is "  + rest(act));
        	
        	for (Cons arg = act ; arg != null ; arg = rest(arg))
        	{
        		if(consp(first(arg)))
        		{
	        		addevent(pq, (Cons)first(arg), time );
        		}
        	}
        }
        ...
        {
        	int voice  = (Integer) second(act);
        	String note = (String) third(act);
        	int d = (Integer) fourth (act);
